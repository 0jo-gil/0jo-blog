...
const Home = ({ data, location: { pathname } }: any) => {
  const {
    allContentfulBlogPost: { totalCount, nodes, group },
  } = data

  const { selectedCategory, categoryList, onChangeCategory } =
    useCategory(group)
    ...
    <Section>
      ...
      <div>
        ...
        <PostList variant={"horizon"}>
          {recentPostList.map((post: any, index: number) => (
            <PostItem key={index} variant="card" post={post} />
          ))}
        </PostList>
      </div>
      ...
      <div>
        <PostList variant="vertical">
          {postList.map((post: any, index: number) => (
            <PostItem key={index} variant="list" post={post} />
          ))}
        </PostList>
      </div>
    </Section>
